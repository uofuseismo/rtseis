#ifndef RTSEIS_TRANSFORMS_DFT_HPP
#define RTSEIS_TRANSFORMS_DFT_HPP 1
#include <memory>
#include <complex>
...
public:
    ...
    /// @param[in] dft  Class from which to initialize this clas.
    DFT(const DFT &dft);
    /// @brief Move constructor.
    /// @param[in,out] dft  The DFT class from which to initialize this class.
    ...
    /// @{
    /// @brief Copy operator.
    /// @param[in] dft  DFT class to copy.
    /// @result A deep copy of the input class.
    DFT& operator=(const DFT &dft);
    /// @brief Move operator.
    /// @param[in,out] dft   The DFT class whose memory will be moved to this.
    ///                      On exit, DFT's behavior is undefined.
    /// @result The memory from dft moved to this.
    DFT& operator=(DFT &&dft) noexcept;
    /// @}
